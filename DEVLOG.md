...
**Triggered by:** Q42 +1 week persistence check FAILED 2026-05-03 13:00 UTC. After a 6-day engine-off gap (last engine-side journal entry 2026-04-27, then VPS reboot triggered during SSH troubleshoot landed at 2026-05-03), the engine cold start re-flagged BOTH `phantom_open_order` (permId 1888063981) AND `phantom_position` (SPY 2 @ 707.72) because both adoption events had decayed out of the 48h `journal_tail` lookback. Re-running with `K2BI_ADOPT_ORPHAN_STOP=1888063981:...` correctly wrote a fresh `orphan_stop_adopted` event but `phantom_position` still refused (Q42 only scoped orphan STOPs, not adopted positions). Operator engaged systemd drop-in `/etc/systemd/system/k2bi-engine.service.d/override.conf` with `Environment="K2BI_ALLOW_RECOVERY_MISMATCH=1"`; engine resumed 2026-05-03T15:32:44Z, position safe throughout. Architect ruling D3 from the original Q42 ship pre-identified this exact failure mode and deferred mitigation to "Phase 4+ work" — now elevated to Phase 3.10 burn-in gating.

**What shipped:**
- `execution/engine/recovery.py` — `EXTENDED_CHECKPOINT_LOOKBACK = timedelta(days=30)` constant; `EXTENDED_CHECKPOINT_EVENT_TYPES = frozenset({"engine_recovered", "orphan_stop_adopted"})`; new `_adopted_orphan_perm_id_to_ticker()` helper extracts permId→ticker map from orphan_stop_adopted events for the position-aware gate.
- `execution/engine/main.py` — new `_read_extended_checkpoints()` helper walks day-files in (narrow_since, ext_since], filters to `EXTENDED_CHECKPOINT_EVENT_TYPES`, returns events sorted oldest-first. Wired into `_run_recovery` such that `journal_tail = extended_checkpoints + narrow_journal_tail`. The variable previously named `lookback_start` was renamed `narrow_lookback_start` to make the narrow/extended split explicit at the call site. `broker_status` retains the narrow 48h window — order-status history is recent broker activity, not a state checkpoint, and widening it would conflict with the existing reconciliation contract for terminal-state classification.
- `execution/engine/recovery.py` reconcile() — position-aware gate on orphan_stop_adopted carry-forward: an adopted permId only suppresses `phantom_open_order` when broker still holds a position in the order's ticker. Closes the cross-mode adversarial review CRITICAL finding (a stale adoption whose underlying position has been closed must NOT mask a dangling broker-side STOP). Same gate also tightens the existing narrow-window behavior — the gap was always present, the carry-forward just widened the time window over which it could manifest.
- `tests/test_engine_recovery_extended_checkpoints.py` (new file) — 14 tests T1-T14:
  - T1-T6: pure-function reconcile tests proving recovery's existing snapshot-reset semantics correctly carry adoptions when the older records are present in journal_tail (T1 position carry, T2 orphan STOP carry, T3 both adoptions → CATCH_UP, T4 no checkpoints + unknown broker state → MISMATCH_REFUSED preserves Q31/Q32 invariants, T5 multiple engine_recovered → latest wins, T6 broker diverges from checkpoint → mismatch still fires);
  - T7-T12: integration tests against on-disk JournalWriter day-files (filter to checkpoint event types, exclude narrow window double-count, exclude events older than 30d, sorted oldest-first, missing day file tolerance, defensive on inverted windows);
  - T13: stale orphan_stop_adopted + closed position → phantom_open_order MUST fire (the new safety property);
  - T14: orphan + held position → carry-forward works as before.

**Tests:** 169 passed across recovery + engine_main + new file (test_engine_recovery + test_engine_recovery_q39 + test_engine_main + test_engine_recovery_extended_checkpoints), 0 regressions. Pre-existing failure in `test_invest_alert_integration.py::MixedJournalTests::test_mixed_events_produce_correct_tier_split` confirmed on `main` baseline, unrelated to this branch.

**Adversarial review:** Cross-mode discipline applied. Two independent Kimi-backed reviews via `scripts/review.sh diff` flagged the stale-orphan-stop suppression risk as CRITICAL/HIGH and converged on the position-aware gate recommendation. Codex pre-commit review (via Agent / `codex:codex-rescue` subagent) reached the `verifying` phase and captured the in-progress note: "The live branch already contains extra recovery logic beyond the 'constants + helper' summary: there is a new permId-to-ticker adoption map and comments explicitly naming the stale-adoption issue. I'm checking whether that implementation actually closes Kimi's scenario and whether it creates any new invariant drift." The Codex turn then wedged (no progress for 21 minutes); operator override "don't wait for codex" — proceeded to commit on the strength of 2x Kimi cross-coverage plus tests T13/T14 directly exercising the safety property. Cross-mode invariant per L-2026-04-26-001 satisfied (planning model = Opus, reviewing model = Kimi cross-mode + Codex partial).

**Production validation:** 2026-05-04T01:21:18Z on the VPS engine. Override drop-in removed (`sudo rm /etc/systemd/system/k2bi-engine.service.d/override.conf && sudo systemctl daemon-reload && sudo systemctl restart k2bi-engine.service`). Engine cold-started clean: `engine_started pid=17333 recovery_status=catch_up mismatch_count=0 strategies_loaded=[spy-first-paper-smoke]`; `engine_recovered status=catch_up adopted_positions=[{ticker:SPY, qty:2, avg_price:707.72}]`. NO `recovery_state_mismatch override_value=1` event in the cold-start sequence — the binary success criterion the original Q42 ship targeted, now actually held across a multi-day gap.

**Out of scope (deferred to follow-ups):**
- Stale `expected_stop_children` carry-forward in extended-window engine_recovered events. Kimi flagged this as MEDIUM. Existing recovery behavior already mostly handles it via Phase B's broker-vs-implied diff (`journal_position_missing_at_broker` fires when broker no longer holds the position the checkpoint expected), but a dedicated coverage test (T15: stale expected_stop_children + closed position) is a worthwhile add for Phase 3.10 burn-in prep.
- Race condition between `_read_extended_checkpoints` and concurrent journal writers. Real risk is low because older day-files (offset > 0) are not being written to — the writer always lands on today's file, which is in the narrow tail. Documented for future hardening, not blocking.
- Stale-checkpoint age threshold beyond the 30-day frozenset. Trade-off: every architect-sanctioned adoption is by definition state the architect ALREADY accepted; an explicit revoke action (separate kickoff if/when needed) is the correct mechanism, not a passive timeout.

**Cross-link:** Architect-side spec at `K2Bi-Vault/wiki/planning/q42-carryforward-fix-kickoff.md`; parent Q42 kickoff at `K2Bi-Vault/wiki/planning/q42-orphan-stop-adoption-kickoff.md` (status flipped to `shipped`); +1 week incident captured at `K2Bi-Vault/wiki/planning/upcoming-sessions.md` finding (x) closure paragraph; architect-side learning at K2B-Vault `System/memory/self_improve_learnings.md` L-2026-05-03-001; request closure at K2B-Vault `System/memory/self_improve_requests.md` REQ-2026-05-03-001; policy-ledger guard at K2B-Vault `wiki/context/policy-ledger.jsonl` (scope=engine-recovery-cold-start, action=require_extended_lookback_for_adoption_events, risk=high). Phase 3.10 5-day unattended burn-in is now unblocked from the engine-recovery side.
...
## 2026-06-11 -- run_full_ship strategy plan + diff reviews made advisory

**What shipped:** `run_full_ship`'s internal PLAN and DIFF reviews went from blocking gates (`_require_review_approved`) to ADVISORY, mirroring the already-shipped limits-apply advisory in `apply_approved_limits` (#13/#14). Both reviews still run and now record `plan_review_advisory` / `diff_review_advisory` events (verdict + log_path); a NEEDS-ATTENTION verdict no longer raises. The now-unused `_require_review_approved` helper was removed (zero remaining callers).

**Motivation (2026-06-10 CDNS evidence):** the strategy plan review is non-deterministic as a hard gate. On the identical operator-approved, sha-bound CDNS first-paper-trade strategy it returned NEEDS-ATTENTION on two consecutive ships (flights `2026-06-08-003` and `2026-06-10-004`) while flagging DIFFERENT findings each run -- run 1: forward-guidance label + empty regime_filter + quarterly-kill + OCO; run 2: earnings-date look-ahead + naked-LMT stop + regime + forward-guidance + dormant-state. "Fix the findings and it passes" is therefore false -- it raises new ones. The whitelist CRITICAL that legitimately blocked the first attempt was already resolved deterministically (A4, CDNS on `instrument_whitelist`, commit `ddd3881`). Strategy quality stays gated by the operator's strategy-approval gate (Stage 9) AND ship-approval gate (the 4th human gate, domain expert in the loop), plus the deterministic capital gates (instrument whitelist, kill-switch, validators config, sha-bound approval token, clean-tree preflight, status:proposed, byte binding, commit/rollback). The LLM reviews were redundant, non-deterministic opinions on top of those. This supersedes the #14 "strategy reviews stay blocking" note, on the strength of the two-run evidence above.

**Adversarial review (Kimi, `.minimax-reviews/2026-06-10T15-53-35Z_diff.json`):** NEEDS-ATTENTION, 3 findings. (1) HIGH "primary/fallback provenance enforcement dropped" -- PUSHED BACK: the shipped limits advisory events also record only verdict + log_path, so this is deliberate symmetry; provenance of a non-blocking advisory opinion is not load-bearing (the deterministic + human gates are model-independent). Re-adding it would diverge from the limits precedent. (2) MEDIUM "no test for rollback after an advisory review" -- FIXED: added `test_advisory_needs_attention_review_then_commit_failure_rolls_back` (advisory NEEDS-ATTENTION plan+diff, then git commit fails -> clean rollback to status:proposed). (3) MEDIUM "dead `_require_review_approved`" -- FIXED: removed it.

**Tests:** reworked `test_failed_plan_review_restores_file_and_does_not_commit` -> `test_needs_attention_plan_review_is_advisory_and_commits` and `test_failed_diff_review_restores_original_bytes_and_does_not_commit` -> `test_needs_attention_diff_review_is_advisory_and_commits` (both now COMMIT + record the advisory event); reworked `test_diff_review_failure_attaches_rollback_result_and_clears_marker` -> `test_commit_failure_attaches_rollback_result_and_clears_marker` (real-gate rollback no longer keyed to the diff verdict); added the finding-#2 advisory-then-rollback test. `pytest tests/test_invest_orchestrator_adapters.py tests/test_invest_ship_strategy.py -q` -> 142 passed, 13 subtests.

**Feature status change:** none (K2Bi vault has no wiki/concepts roadmap; infrastructure calibration of an already-shipped adapter).
